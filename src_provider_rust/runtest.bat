...
REM Enable verbose provider logging unless caller already configured it
if "%RUST_LOG%"=="" (
    set RUST_LOG=akv_provider=debug,reqwest=warn
)
if "%RUST_LOG_STYLE%"=="" (
...
if exist %TEMP_FOLDER%\myrsakey_pub.pem (
    echo RSA public key already exported at %TEMP_FOLDER%\myrsakey_pub.pem
) else (
    echo Running: openssl pkey -provider akv_provider -provider default -in %RSA_PROVIDER_PATH% -pubout -out %TEMP_FOLDER%\myrsakey_pub.pem
    openssl pkey -provider akv_provider -provider default -in %RSA_PROVIDER_PATH% -pubout -out %TEMP_FOLDER%\myrsakey_pub.pem
    if errorlevel 1 goto :error
